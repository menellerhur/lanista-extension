// Depends on: extension/page.js (showExtensionPage), database/page.js (showDatabasePage)
...
function updateChallengeWarning() {
  ...
  if (challengeHidden && showIcon && isReady) {
    if (!challengesLink.querySelector('.ext-challenge-warning')) {
      ...
      const badge = challengesLink.querySelector('.badge-notify');
      if (badge) {
        badge.parentNode.insertBefore(icon, badge);
      } else {
        challengesLink.appendChild(icon);
      }
    }
  } else {
