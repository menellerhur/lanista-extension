...
// Used by: pages/extension/page.js, pages/database/page.js
...
function getPlanGladiatorLink() {
  return document.querySelector(".ext-plan-gladiator-link .sidebar-nav-link");
}
...
function hideCustomPage(skipHistory = false) {
  ...
  getPlanGladiatorLink()?.classList.remove("sidebar-nav-link-active");
...
document.addEventListener("click", e => {
  ...
  if (!link) return;
  if (link === getExtensionLink() || link === getDatabaseLink() || link === getNotificationsLink() || link === getPlanGladiatorLink()) return;
...
window.addEventListener("popstate", () => {
  ...
  } else if (hash === "#planera-gladiator") {
...
window.addEventListener("ext:navigate", (e) => {
  ...
  if (path.includes("#extension") || path.includes("#database") || path.includes("#notifications") || path.includes("#planera-gladiator")) {
    return;
  }
  hideCustomPage(true);
});
