...
//             common/items-data.js (itemsGetBonus, itemsGetRaceReqs)
...
function itemsGetSignedNumber(val) {
...
function itemsGetAgeReq(item) {
  if (!item.requirements) return null;
  const r = item.requirements.find(req => req.age && Array.isArray(req.age) && req.age.length > 0);
  return r ? r.age : null;
}

const ITEMS_AGE_LABELS = {
  "0": "Ung",
  "1": "Vuxen",
  "2": "Medelålders",
  "3": "Gammal",
...
function itemsFormatAgeReq(ageCodes) {
  if (!ageCodes || !ageCodes.length) return "–";
  return ageCodes.map(code => ITEMS_AGE_LABELS[code] || code).join(", ");
}
...
function itemsGetAllAges(items) {
  const ages = new Set();
  for (const item of items) {
    const codes = itemsGetAgeReq(item);
    if (codes) codes.forEach(c => ages.add(c));
  }
  return [...ages].sort((a, b) => Number(a) - Number(b));
}
  ...
  return items.filter(item => {
    for (const [key, f] of Object.entries(itemsColFilters)) {
      ...
      if (key === "_damage_range") {
        ...
        if (!f.ages?.length) continue;
        const ageCodes = itemsGetAgeReq(item) || [];
        const passes = f.ages.some(a => a === "__none__" ? ageCodes.length === 0 : ageCodes.includes(a));
        if (!passes) return false;
      }
    }
    return true;
