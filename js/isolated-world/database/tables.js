...
function itemsBuildItemTooltip(item) {
  ...
  if (item.is_consumable) {
    ...
    if (item.xp > 0)          mods.push(`<strong>${item.xp}</strong> erfarenhet`);
    if (item.given_coins > 0) mods.push(`<strong>${item.given_coins}</strong> silvermynt`);
    if (item.popularity)      mods.push(`<strong>${item.popularity}</strong> rykte`);
    if (item.rounds > 0)      mods.push(`<strong>${item.rounds}</strong> rundor`);
  ...
  if (item.asset?.url) {
    rightRows.push(`<img style="max-height:120px;height:auto;width:auto;margin-top:24px" src="${esc(item.asset.url)}">`);
  }
