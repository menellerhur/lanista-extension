...
var ITEMS_BASE_COLS = {
  ...
  weapon_parera:   ["required_level", "is_two_handed", "_damage_range", "durability", "durability_scaling", "_actions_display", "weight", "req_strength", "req_vf"],
  ...
  shield:          ["required_level", "absorption", "durability", "max_blocks_per_round", "weight", "req_strength", "req_vf", "_races", "_profession", "_merchantName", "_merchantPrice"],
  armor:           ["type_name", "required_level", "base_block", "weight", "req_strength", "_races", "_profession", "_merchantName", "_merchantPrice"],
...
function itemsRenderDurabilityHeader(key) {
  ...
  if (itemsGladiatorVf > 0) {
    ...
    const btnTh       = `<th class="py-1 pr-1 text-xs text-muted-foreground cursor-pointer select-none ext-bv-group-th ext-bv-btn-th${activeCls}" data-sort-col="durability"><div class="ext-th-actions">${menuBtn}</div></th>`;
    return itemsTh("BV", "durability", "right", "Brytvärde", { noMenu: true, filterActive }) +
           `<th class="${sepCls} ext-bv-group-th" data-sort-col="durability">(</th>` +
           `<th colspan="3" class="${bonusCls} ext-bv-group-th" data-sort-col="durability">Bonus</th>` +
           `<th class="${sepCls} ext-bv-group-th" data-sort-col="durability">)</th>` +
           btnTh;
  }
...
function itemsRenderAbsorptionHeader(key) {
  ...
  if (itemsGladiatorStr > 0) {
    ...
    const btnTh       = `<th class="py-1 pr-1 text-xs text-muted-foreground cursor-pointer select-none ext-bv-group-th ext-bv-btn-th${activeCls}" data-sort-col="absorption"><div class="ext-th-actions">${menuBtn}</div></th>`;
    return itemsTh("ABS", "absorption", "right", "Absorbering", { noMenu: true, filterActive }) +
           `<th class="${sepCls} ext-bv-group-th" data-sort-col="absorption">(</th>` +
           `<th colspan="3" class="${bonusCls} ext-bv-group-th" data-sort-col="absorption">Bonus</th>` +
           `<th class="${sepCls} ext-bv-group-th" data-sort-col="absorption">)</th>` +
           btnTh;
  }
...
var ITEMS_ALL_COLUMNS = {
  ...
  "max_blocks_per_round":    { label: "Max antal blockeringar per runda", shortLabel: "Max block",       tooltip: "Max antal blockeringar per runda", align: "right",  type: "number",            getValue: i => i.max_blocks_per_round ?? null },
  ...
  "xp":                      { label: "Erfarenhet",                       shortLabel: "EP",              tooltip: null,                               align: "right",  type: "number",            getValue: i => i.xp ?? null },
  "given_coins":             { label: "Silvermynt",                       shortLabel: "SM",              tooltip: null,                               align: "right",  type: "number",            getValue: i => i.given_coins ?? null },
  "popularity":              { label: "Bonus Rykte",                      shortLabel: "Rykte+",          tooltip: "Bonus Rykte",                      align: "right",  type: "number",            getValue: i => i.popularity ?? null },
  "rounds":                  { label: "Rundor",                                                          tooltip: null,                               align: "right",  type: "number",            getValue: i => i.rounds ?? null },
